...
```{r}
first_stage <- lm(proposed_turbine_3km~log_wind_power + mindistlake + latitude + longitude + mindistlake_sq + long_sq + lat_sq + long_lat + district_id, data = matched_data)
...
*Response:* The F-statistic is reported as 38.21. As this is larger than 10, this suggests a that wind power is a strong instrument in the study. However, this can change with omitted variable bias. A weak instrument in this case would suggest that wind power is not a strong predictor of turbine placement. This would mean the Green Energy Act's implementation is being shaped by considerations beyond renewable energy. 
...
```{r}
second_stage <- lm(change_liberal~proposed_turbine_3km_HAT+ mindistlake + latitude + longitude + mindistlake_sq + long_sq + lat_sq + long_lat + district_id, data = matched_data)
...
*Response:* The estimated effect of proposed turbines on liberal vote share is negative, which suggests that voters retaliate against proposed turbines in their areas. However, this estimate is not shown to be significant, so this result could have been achieved randomly. IV identifies the local average treatment effect within areas that received turbine proposals because of high wind power. This means that this treatment effect is only interpretable within these areas and not necessarily the whole population.
...
fit_2sls <- ivreg(change_liberal ~ proposed_turbine_3km + mindistlake + latitude + longitude + mindistlake_sq + long_sq + lat_sq + long_lat + district_id | log_wind_power + mindistlake + latitude + longitude + mindistlake_sq + long_sq + lat_sq + long_lat + district_id, data = matched_data)
